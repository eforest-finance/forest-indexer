...
public class SeedCreatedProcessorTests : ForestIndexerPluginTestBase
{
    ...
    private const string OwnedSymbolNFT = "AAA-0";
    ...
    public SeedCreatedProcessorTests()
    {
    ...
    [Fact]
    public async Task TestConvertToElf()
    {
        DecimalUntil.ConvertToElf(100000000).ShouldBe(1);
    }
    ...
    [Fact]
    public async Task HandleDealtProcessorAsync_Test()
    {
        ...
        SeedCreated seedCreated = new SeedCreated
        {
            Symbol = "SEED-1",
            OwnedSymbol = "AAA-0",
        ...
        var logEventInfo = LogEventHelper.ConvertAElfLogEventToLogEventInfo(seedCreated.ToLogEvent());
        ...
        //step5: check result
        var seedSymbolId = IdGenerateHelper.GetSeedSymbolId(chainId, OwnedSymbolNFT);
        ...
        tsmSeedSymbolIndex.Symbol.ShouldBe(seedCreated.OwnedSymbol);
        tsmSeedSymbolIndex.SeedSymbol.ShouldBe(seedCreated.Symbol);
        tsmSeedSymbolIndex.Owner.ShouldBe(seedCreated.To.ToBase58());
        tsmSeedSymbolIndex.Status.ShouldBe(SeedStatus.UNREGISTERED);
        tsmSeedSymbolIndex.ExpireTime.ShouldBe(seedCreated.ExpireTime);
    }
}